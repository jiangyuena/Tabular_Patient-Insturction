...
class LightningModel(pl.LightningModule):
    ...
    def forward(self, input_ids,input_ids_tabular, attention_mask,attention_mask_tabular, decoder_attention_mask, labels=None, relevant_embs_list=None):
        """ forward step """
        output = self.model(
            input_ids,
    ...
    def feedforward_step(self, batch):
        ...
        input_ids_tabular=batch["tabular_text_input_ids"]
        attention_mask = batch["source_text_attention_mask"]
        attention_mask_tabular=batch["tabular_text_attention_mask"]
        labels = batch["labels"]
...
def load_csv(
        path, 
        columns={"discharge_instruction": "target_text", "discharge_summary":"source_text", "merged_data": "tabular_text"},
        extract_columns=['key', 'source_text', 'tabular_text','target_text'],
    ):
    import pandas as pd
    df = pd.read_csv(path)
    df = df.rename(columns=columns)
...
def load_csv(
        path,
        columns={"discharge_instruction": "target_text", "discharge_summary":"source_text",
                 "merged_data": "tabular_text"},
        extract_columns=['key', 'source_text', 'tabular_text','target_text'],
    ):
    import pandas as pd
    df = pd.read_csv(path)
    merge_columns = [col for col in df.columns if col not in ['key', 'discharge_instruction', 'discharge_summary']]
    df['merged_data'] = df[merge_columns].fillna('').astype(str).apply(lambda x: ' '.join(x), axis=1)
    df = df.rename(columns=columns)
    df = df[extract_columns]
    return df
...
class SimpleT5:
    ...
    def predict(
        ...
        input_ids_tabular=self.tokenizer.encode(
            tabular_text, return_tensors="pt", add_special_tokens=True
        )
        input_ids_tabular = input_ids_tabular.to(self.device)
